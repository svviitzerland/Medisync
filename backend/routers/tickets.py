...
async def create_ticket(
    ...
    try:
        ...
        if requires_inpatient:
            ...
            active_teams = set([n["team_id"] for n in nurses_res.data]) if nurses_res.data else {1, 2, 3}
            
            tickets_res = supabase.table("tickets").select("nurse_team_id").in_("status", ["inpatient", "operation"]).execute()
        ...
        return {"status": "success", "ticket": data[1][0] if data[1] else None, "assigned_nurse_team": nurse_team_id}
...
async def complete_checkup(ticket_id: int, doctor_note: str = Body(...), require_pharmacy: bool = Body(False), requires_inpatient: bool = Body(False)):
    """
